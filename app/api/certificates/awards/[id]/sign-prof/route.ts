...
import { canSignAwardAsProf, canViewAward } from "@/app/lib/certificates-auth";
import { loadAward, loadProgram, saveAward } from "@/app/lib/certificates-storage";
import { signAwardAsProf } from "@/app/lib/certificates-workflow";
...
export async function POST(_req: Request, { params }: Params) {
  ...
  if (!canSignAwardAsProf(award, gate.ctx.userId)) {
    return NextResponse.json({ error: "Signature non autorisée." }, { status: 403 });
  }
