...
import {
  copyPendingFileToRequest,
  deletePendingRequestPrefix,
  loadPendingRequestMeta,
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!meta) {
      return redirectToMerci(req, { erreur: "lien_invalide" });
    }
    ...
    for (let i = 0; i < meta.attachmentKeys.length; i++) {
      ...
      const copied = await copyPendingFileToRequest(
        key,
        id,
        m.id,
        m.fileName,
    ...
    try {
      ...
      console.error("Request confirm notification error:", mailError);
    }
