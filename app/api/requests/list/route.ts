...
import {
  computeStaffBoardColumn,
  isCorbeilleBranchId,
  isVisibleOnStaffBoard,
  normalizeRequestBranchId,
  normalizeRequestEmail,
} from "@/app/lib/requests-board";
import {
  getAllBranchStaffEmails,
  getDelegateTargetEmailsForRequest,
  getRequestsIndex,
...
function hasStaffBoardAccess(roles: string[], email: string) {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const sortDesc = (a: (typeof index)[number], b: (typeof index)[number]) =>
    ...
    /** Tableau unique : corbeille + pile de mon service + mes prises en charge (pas « toutes » / « ma file »). */
    if (scope === "board" || scope === "all" || scope === "my_queue") {
      ...
      const visible = index.filter((r) =>
        isVisibleOnStaffBoard(
          ...
          userEmail,
          allStaff,
          isLeaderForRequestBranch(r.assignedTo.routeId, r.assignedTo.unit, userEmail),
        ),
      );
