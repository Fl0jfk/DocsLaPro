...
import { SignedIn, UserButton, SignOutButton, useAuth } from '@clerk/nextjs';
...
export default function Header() {
  ...
  const excludedPrefixes = ['/portesouvertes','/simulateurTarifs'];
  const isExcluded = excludedPrefixes.some(prefix => pathname?.startsWith(prefix));
  if (isExcluded) return null;
  if (!isClient) return null;
  const targetWidth = menuOpened ? (isSignedIn ? 190 : 100) : 56;
  return (
    <motion.header variants={{ visible: { y: 0 }, hidden: { y: -120 } }} animate={hidden ? "hidden" : "visible"} transition={{ duration: 0.2 }} className="fixed top-6 right-6 z-[100] max-w-[1500px] mx-auto">
      <motion.div animate={{ width: targetWidth }} transition={{ type: "spring", stiffness: 500, damping: 40 }} className="bg-white/40 backdrop-blur-xl border border-white/20 shadow-xl h-[56px] rounded-full overflow-hidden flex items-center justify-end">
