...
export default function DocumentsPage() {
  ...
  // --- Nouveaux états pour l'upload ---
  const [uploading, setUploading] = useState(false);
  const [selectedDest, setSelectedDest] = useState("");

  // Fonction pour charger les données (extraite pour pouvoir la rappeler après l'upload)
  ...
  // --- Fonction d'upload avec nettoyage du cache ---
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        // 1. On vide le cache localStorage pour le préfixe où on a uploadé
        // On nettoie le cache du dossier cible ET du dossier actuel pour être sûr
        ...
        // 2. On relance le fetch pour mettre à jour l'affichage immédiatement
        fetchDocuments(currentPrefix, true);
        
        // Optionnel : Reset l'input file
        e.target.value = "";
      } else {
    ...
    <main className="flex flex-col gap-4 p-4 w-full mx-auto max-w-[1000px] sm:pt-[10vh]">
      ...
      <div className="bg-gray-50 border border-gray-200 p-4 rounded-xl mb-4 flex flex-col sm:flex-row gap-4 items-end">
        <div className="flex flex-col gap-1 w-full sm:w-1/2">
          ...
          >
            ...
            {/* On affiche les dossiers disponibles dans la vue actuelle */}
        ...
        <div className="flex flex-col gap-1 w-full sm:w-1/2">
          ...
          <input 
            ...
            disabled={!selectedDest || uploading}
      ...
      {items.length > 0 ? (
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-6 relative">
          {/* Petit overlay de chargement discret pour le refresh */}
          ...
          {items.map((item, idx) => (
            <div key={idx} onClick={() => item.type === "folder" ? enterFolder(item.path) : handleDownload(item.path)} className="cursor-pointer flex flex-col items-center max-h-[150px]">
              ...
              <div className="text-center text-xs font-medium break-words w-full px-1 line-clamp-2">
