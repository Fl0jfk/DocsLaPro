...
export default function InventoryPage() {
  ...
  const [temporaryInventory, setTemporaryInventory] = useState<Record<string, any>>({}); // Inventaire temporaire pour les changements
  ...
  const [isPasswordCorrect, setIsPasswordCorrect] = useState<boolean>(false); // Validation du mot de passe
  const [modificationsInProgress, setModificationsInProgress] = useState<any[]>([]); // Nouveau tableau pour les modifications en cours
  useEffect(() => {
    fetch("/api/inventory")
      .then((res) => {
        if (!res.ok) {
