...
"[project]/node_modules/api-ecoledirecte/node_modules/node-fetch/lib/index.mjs [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
"[project]/node_modules/api-ecoledirecte/utils/dateToString.js [app-route] (ecmascript)", ((__turbopack_context__, module, exports) => {

module.exports = (date)=>/^\d{4}-\d{2}-\d{2}$/.test(date) ? date : `${date.getFullYear()}-${date.getMonth() < 9 ? '0' : ''}${date.getMonth() + 1}-${date.getDate() < 10 ? '0' : ''}${date.getDate()}`;
}),
"[project]/node_modules/api-ecoledirecte/classes/Eleve.js [app-route] (ecmascript)", ((__turbopack_context__, module, exports) => {

const Session = __turbopack_context__.r("[project]/node_modules/api-ecoledirecte/classes/Session.js [app-route] (ecmascript)");
const dateToString = __turbopack_context__.r("[project]/node_modules/api-ecoledirecte/utils/dateToString.js [app-route] (ecmascript)");
module.exports = class Eleve {
    ...
    */ constructor(session, compte){
        ...
        this.classe = classe ? {
            ...
            nom: profile?.classe?.libelle,
            id: profile?.classe?.id
        };
    ...
    */ fetchVieScolaire() {
        return new Promise((resolve, reject)=>{
            this.session.request(`/eleves/${this.id}/viescolaire.awp?verbe=get`).then((data)=>resolve(data)).catch((err)=>reject(err));
        });
    }
    /**
    * @returns {Promise}
    */ fetchVieDeClasse() {
        return new Promise((resolve, reject)=>{
            this.session.request(`/Classes/${this.classe.id}/viedelaclasse.awp?verbe=get`).then((data)=>resolve(data)).catch((err)=>reject(err));
        });
    }
    /**
    * @returns {Promise}
    */ fetchNotes() {
        return new Promise((resolve, reject)=>{
            this.session.request(`/eleves/${this.id}/notes.awp?verbe=get`).then((data)=>resolve(data)).catch((err)=>reject(err));
        });
    }
    /**
    ...
    */ fetchMessages({ query, folder, orderBy, order, page, perPage, ...options } = {}) {
        ...
        return new Promise((resolve, reject)=>{
            this.session.request(`/eleves/${this.id}/messages.awp?${queryString}`).then((data)=>resolve(data)).catch((err)=>reject(err));
        });
    }
    /**
    *
    ...
    */ fetchMessage(messageId, mode = 'destinataire') {
        ...
        return new Promise((resolve, reject)=>{
            this.session.request(`/eleves/${this.id}/messages/${messageId}.awp?verbe=get&mode=${mode}`).then((data)=>resolve(data)).catch((err)=>reject(err));
        });
    }
    /**
    *
    ...
    */ fetchEDT(start = new Date(), end = new Date(start.getTime() + 86400000)) {
        ...
        return new Promise((resolve, reject)=>{
            this.session.request(`/E/${this.id}/emploidutemps.awp?verbe=get`, {
                dateDebut: start,
                dateFin: end,
                avecTrous: false
    ...
    */ fetchAgenda(jour) {
        return new Promise((resolve, reject)=>{
            this.session.request(`/Eleves/${this.id}/cahierdetexte${jour ? '/' + dateToString(jour) : ''}.awp?verbe=get`).then((data)=>resolve(data)).catch((err)=>reject(err));
        });
    }
    /**
    * @returns {Promise}
    */ fetchDocuments() {
        return new Promise((resolve, reject)=>{
            this.session.request(`/elevesDocuments.awp?archive=&verbe=get`).then((data)=>resolve(data)).catch((err)=>reject(err));
        });
    }
};
}),
"[project]/node_modules/api-ecoledirecte/utils/errors.json (json)", ((__turbopack_context__) => {

__turbopack_context__.v(JSON.parse("{\"210\":\"Aucune donnée disponible\",\"240\":\"La charte d'utilisation n'a pas été acceptée\",\"250\":\"Authentification à deux facteurs requise\",\"505\":\"Identifiant ou mot de passe invalide\",\"516\":\"L'établissement a fermé EcoleDirecte\",\"518\":\"Impossible de se connecter : la fiche utilisateur n'existe pas\",\"520\":\"Token invalide\",\"525\":\"Session expirée\",\"535\":\"L'établissement a fermé EcoleDirecte\",\"default\":\"Une erreur est survenue\"}"));}),
"[project]/node_modules/api-ecoledirecte/classes/Session.js [app-route] (ecmascript)", ((__turbopack_context__, module, exports) => {

const fetch = __turbopack_context__.r("[project]/node_modules/api-ecoledirecte/node_modules/node-fetch/lib/index.mjs [app-route] (ecmascript)");
const UserAgent = __turbopack_context__.r("[project]/node_modules/user-agents/dist/index.js [app-route] (ecmascript)");
const Eleve = __turbopack_context__.r("[project]/node_modules/api-ecoledirecte/classes/Eleve.js [app-route] (ecmascript)");
const errors = __turbopack_context__.r("[project]/node_modules/api-ecoledirecte/utils/errors.json (json)");
module.exports = class Session {
    ...
    */ fetchGTKToken() {
        return new Promise(async (resolve, reject)=>{
            ...
            const match = res?.headers.get('set-cookie')?.match(/GTK=([^;]+);.+domain=ecoledirecte\.com, (.+)=\1/);
            if (!match) return reject({
                message: 'Impossible de récupérer le token GTK.'
            });
            resolve([
                match[1],
                `GTK=${match[1]}; ${match[2]}=${match[1]}`
            ]);
        });
    }
    /**
    ...
    */ fetch2FAQuestion(identifiant, motdepasse) {
        if (!identifiant || !motdepasse) throw new Error("Nom d'utilisateur ou mot de passe non renseigné.");
        return new Promise(async (resolve, reject)=>{
            const success = await this.login(identifiant, motdepasse).then(()=>{
                reject({
            ...
            if (!success) return;
            this.request('/connexion/doubleauth.awp?verbe=get').then((data)=>resolve({
                    rawQuestion: data.question,
                    rawPropositions: data.propositions,
                    question: Buffer.from(data.question, 'base64').toString('utf8'),
                    propositions: data.propositions.map((proposition)=>Buffer.from(proposition, 'base64').toString('utf8'))
    ...
    */ fetch2FACreds(choix) {
        ...
        return new Promise((resolve, reject)=>{
            this.request('/connexion/doubleauth.awp?verbe=post', {
                choix
            }).then((data)=>resolve(data)).catch((err)=>reject(err));
        });
    }
    ...
    */ login(identifiant, motdepasse, fa) {
        ...
        return new Promise(async (resolve, reject)=>{
            ...
            this.request('/login.awp', {
                ...
                const type = account.typeCompte === '1' || account.typeCompte === '2' ? 'FAMILLE' : account.typeCompte === 'E' ? 'ELEVE' : null;
                if (!type) {
                    reject({
...
"[project]/node_modules/api-ecoledirecte/index.js [app-route] (ecmascript)", ((__turbopack_context__, module, exports) => {

module.exports.Session = __turbopack_context__.r("[project]/node_modules/api-ecoledirecte/classes/Session.js [app-route] (ecmascript)");
module.exports.Eleve = __turbopack_context__.r("[project]/node_modules/api-ecoledirecte/classes/Eleve.js [app-route] (ecmascript)");
}),
];

//# sourceMappingURL=node_modules_5b537df1._.js.map